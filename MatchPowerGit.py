...
G=int(sys.argv[2])  # How many genes you expect in genetic arichitecture
kk=int(sys.argv[3])  # How many matches do you want to detect
...
precision=int(sys.argv[7])  # Simulation will be repeated 100*precision times- recomended=100 higher- more time, more precise estimates
...
pDis=pnew
sp=sum(pDis)
pDisdensity=pDis/sp # normalized- to reflect probabilities
...
a=Bonferpower(N,G,alpha/T,fk,pDis,pDisCum,pmiss,precision*100)

print "Probability of finding "+str(kk)+" matches = "+ str(len(a[0][a[0]>=kk])/(precision*100.0))

print "Mean number of matches = "+str(a[0].mean())
